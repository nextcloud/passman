OC.L10N.register(
    "passman",
    {
    "Error while saving field" : "Viga välja salvestamisel",
    "A Passman item has been created, modified or deleted" : "Passmani kirje on loodud, muudetud või kustutatud",
    "A Passman item has expired" : "Passmani kirje on aegunud",
    "A Passman item has been shared" : "Passmani kirje on jagatud",
    "A Passman item has been renamed" : "Passmani kirje nimi on muudetud",
    "%1$s has been created by %2$s" : "%2$s on lisanud „%1$s“ kirje",
    "You created %1$s" : "Sa lisasid „%1$s“ kirje",
    "%1$s has been updated by %2$s" : "%2$s on uuendanud „%1$s“ kirjet",
    "You updated %1$s" : "Sa uuendasid „%1$s“ kirjet",
    "%2$s has revised %1$s to the revision of %3$s" : "%2$s muutis „%1$s“ versiooni tagasi versiooniks „%3$s“",
    "You reverted %1$s back to the revision of %3$s" : "Sa muutsid „%1$s“ versiooni tagasi versiooniks „%3$s“",
    "%3$s has renamed %1$s to %2$s" : "%3$s muutis „%1$s“ nime, uus nimi on „%2$s“",
    "You renamed %1$s to %2$s" : "Sa muutsid „%1$s“ nime, uus nimi on „%2$s“",
    "%1$s has been deleted by %2$s" : "%2$s kustutas „%1$s“ kirje",
    "You deleted %1$s" : "Sa kustutasid „%1$s“ kirje",
    "%1$s has been recovered by %2$s" : "%2$s on taastanud „%1$s“ kirje",
    "You recovered %1$s" : "Sa taastasid „%1$s“ kirje",
    "%1$s has been permanently deleted by %2$s" : "%2$s kustutas „%1$s“ kirje jäädavalt",
    "You permanently deleted %1$s" : "Sa kustutasid „%1$s“ kirje jäädavalt",
    "The password of %1$s has expired, renew it now." : "„%1$s“ salasõna on aegunud  - palun uuenda teda kohe.",
    "%1$s has been shared with %2$s" : "„%1$s“ kirje on jagatud kasutajaga „%2$s“",
    "You received a share request for %1$s from %2$s" : "Sa said kasutajalt „%2$s“ „%1$s“ kirje jagamispäringu",
    "%s has been shared with a link" : "„%s“ kirje on lingiga jagatud",
    "User not found" : "Kasutajat ei leidu",
    "Generating sharing keys (%s/2)" : "Loon jagatud võtmeid (%s/2)",
    "Incorrect vault password!" : "Andmehoidla vale salasõna",
    "Passwords do not match" : "Salasõnad ei kattu",
    "General" : "Üldine",
    "Custom Fields" : "Kohandatud väljad",
    "Please fill in a label." : "Palun anna kirjele nimi.",
    "Please fill in a value." : "Palun sisesta väärtus.",
    "Error loading file" : "Viga faili laadimisel",
    "An error occurred during decryption" : "Dekrüptimisel tekkis viga.",
    "Credential created!" : "Salasõnakirje on loodud!",
    "Credential deleted" : "Salasõnakirje on kustutatud",
    "Credential updated" : "Salasõnakirje on uuendatud",
    "Credential recovered" : "Salasõnakirje on taastatud",
    "Credential destroyed" : "Salasõnakirje on hävitatud",
    "Error downloading file, you probably have insufficient permissions" : "Viga faili allaalaadimisel, sul ilmselt pole piisavalt õigusi",
    "An error occurred" : "Tekkis viga",
    "Invalid QR code" : "Vigane QR-kood",
    "Starting export" : "Alustan eksportimist",
    "Decrypting credentials" : "Dekrüptin salasõnakirjeid",
    "Failed to decrypt {{credential}}" : "Dekrüptimine ei õnnestunud {{credential}}",
    "Done" : "Valmis",
    "File read." : "Fail on loetud.",
    "Proceed with the following steps to import your file" : "Oma faili importimiseks jätka niiviisi",
    "Skipping unlabeled credential" : "Jätan vahele nimeta salasõnakirje",
    "Adding {{credential}}" : "Lisan: {{credential}}",
    "Added {{credential}}" : "Lisasin: {{credential}}",
    "Skipping credential, missing label on line {{line}}" : "Jätan salasõnakirje vahele, real {{line}} on nimi puudu",
    "Parsed {{num}} credentials, starting to import" : "Töötlesin {{num}} salasõnakirjet, alustan importimist",
    "Importing" : "Impordin",
    "Start import" : "Alusta importimist",
    "Select CSV file" : "Vali csv-fail",
    "Parsed {{rows}} lines from CSV file" : "Töötlesin csv-failist {{rows}} rida",
    "Skip first row" : "Jäta esimene rida vahele",
    "You need to assign the label field before you can start the import." : "Enne importimise alustamist pead määrama nime välja.",
    "The first 5 lines of the CSV are shown." : "Näitan CSV-faili 5 esimest rida.",
    "Assign the proper fields to each column." : "Määra igale veerule õiged väljad.",
    "Example of imported credential" : "Imporditud salasõnakirje näide",
    "Missing an importer? Click here to try it with the generic CSV importer." : "Kas importer on puudu? Klõpsi siin ja proovi importida üldise CSV-failist importijaga.",
    "Go back to importers." : "Tagasi importijate juurde.",
    "Revision deleted" : "Varasem versioon on kustutatud",
    "Revision restored" : "Varasem versioon on taastatud",
    "Save in Passman" : "Salvesta Passmanis",
    "Settings saved" : "Seadistused on salvestatud",
    "General settings" : "Üldised seadistused",
    "Password audit" : "Salasõnade audit",
    "Password settings" : "Salasõnade seadistused",
    "Import credentials" : "Impordi salasõnakirjeid",
    "Export credentials" : "Ekspordi salasõnakirjeid",
    "Since all data has to be downloaded, re-encrypted and uploaded again when changing the password, this process is error-prone. It is strongly recommended to have an updated backup of your vault before changing the password." : "Kuna salasõna muutmisel on kõik andmed esmalt vaja alla laadida, uuesti krüptida ja siis üles laadida, on kogu see protsess aldis vigade tekkeks. Me tungivalt soovitame, et esmalt teed andmehoidlast varukoopia ja alles siis asud salasõna muutma.",
    "I understand the risks" : "Ma mõistan riske",
    "Sharing" : "Jagamine",
    "Are you sure you want to leave? This will destroy all your credentials" : "Kas oled kindel, et soovid lahkuda? Sellega kustuvad kõik sinu salasõnakirjed",
    "Old password field incorrect!" : "Vana salasõna väli pole korrektne!",
    "New password does not match!" : "Uus salasõna ei kattu!",
    "Please log in with your new vault password" : "Palun logi sisse oma uue andmehoidla salasõnaga",
    "Share with users and groups" : "Jaga kasutajate ja gruppidega",
    "Share link" : "Jagamislink",
    "Are you sure you want to leave? This will corrupt this credential" : "Kas oled kindel, et soovid lahkuda? Sellega läheb sinu see salasõnakirje katki",
    "Credential unshared" : "Kasutajakonto/salasõna jagamine on lõppenud",
    "Credential shared" : "Kasutajakonto/salasõna on jagatud",
    "Saved!" : "Salvestatud!",
    "Poor" : "Kehv",
    "Weak" : "Nõrk",
    "Good" : "Hea",
    "Strong" : "Tugev",
    "Toggle visibility" : "Lülita nähtavus sisse/välja",
    "Copy to clipboard" : "Kopeeri lõikelauale",
    "Copied to clipboard!" : "Kopeeritud lõikelauale!",
    "Generate password" : "Loo salasõna",
    "Copy password to clipboard" : "Kopeeri salasõna lõikelauale",
    "Password copied to clipboard!" : "Salasõna on kopeeritud lõikelauale!",
    "Complete" : "Valmis",
    "Username" : "Kasutajanimi",
    "Repeat password" : "Korda salasõna",
    "Add tag" : "Lisa silt",
    "Pick an icon" : "Vali ikoon",
    "Search icons" : "Otsi ikoone",
    "Upload a custom icon:" : "Laadi üles endavalitud ikoon:",
    "Use this icon" : "Kasuta seda ikooni",
    "Delete current icon" : "Kustuta praegune ikoon",
    "Get icon from page" : "Laadi ikoon veebilehelt",
    "This may take a few seconds …" : "Selleks võib kuluda mõni hetk…",
    "There was an error fetching the icon!" : "Ikooni laadimisel tekkis viga!",
    "Selected icon" : "Valitud ikoon",
    "Field label" : "Välja silt",
    "Field value" : "Välja väärtus",
    "Choose a file" : "Vali fail",
    "Text" : "Tekst",
    "File" : "Fail",
    "Add" : "Lisa",
    "Value" : "Väärtus",
    "Type" : "Tüüp",
    "Actions" : "Tegevused",
    "Empty" : "Tühi",
    "Filename" : "Failinimi",
    "Upload date" : "Üleslaadimise kuupäev",
    "Size" : "Suurus",
    "Upload or enter your OTP secret" : "Laadi üles või sisesta ühekordse salasõna salakood",
    "Current OTP settings" : "Praegused ühekordse salasõna seadistused",
    "Issuer" : "Väljastaja",
    "Secret" : "Saladus",
    "Digits" : "Numbrid",
    "Period" : "Tühik",
    "Expiration date" : "Aegumise kuupäev",
    "No expiration date set" : "Aegumise kuupäeva pole määratud",
    "Renew interval" : "Uuendamise välp",
    "Disabled" : "Keelatud",
    "Day(s)" : "Päev(a)",
    "Week(s)" : "Nädal(at)",
    "Month(s)" : "Kuu(d)",
    "Year(s)" : "Aasta(t)",
    "Password generation settings" : "Salasõnaloome seadistused",
    "Password length" : "Salasõna pikkus",
    "Minimum amount of digits" : "Numbrite miinimumarv",
    "Generate password on creation" : "Uue kirje loomisel tekita kohe ka salasõna ",
    "Use uppercase letters" : "Kasuta suurtähti",
    "Use lowercase letters" : "Kasuta väiketähti",
    "Use numbers" : "Kasuta numbreid",
    "Use special characters" : "Kasuta erimärke",
    "Avoid ambiguous characters" : "Väldi mitmetähenduslikke tähemärke",
    "Require every character type" : "Eelda kõiki tüüpi tähemärkide kasutamist",
    "Export type" : "Eksportimise tüüp",
    "Export" : "Ekspordi",
    "Enter vault password to confirm export." : "Eksportimise kinnitamiseks sisesta andmehoidla salasõna.",
    "Rename vault" : "Muuda andmehoidla nime",
    "New vault name" : "Andmehoidla uus nimi",
    "Change" : "Muuda",
    "Change vault key" : "Muuda andmehoidla võtit",
    "Old vault password" : "Andmehoidla vana salasõna",
    "New vault password" : "Andmehoidla uus salasõna",
    "Repeat new vault password" : "Korda andmehoidla uut salasõna",
    "Please wait your vault is being updated, do not leave this page." : "Palun oota, kuni andmehoidla saab uuendatud ja ära lahku siit lehelt.",
    "Processing" : "Töötlemisel",
    "Total progress" : "Edenemine kokku",
    "About Passman" : "Rakenduse teave: Passman",
    "Version" : "Versioon",
    "Donate to support development" : "Toeta arendust rahaliselt",
    "Bookmarklet" : "Käivitatava koodiga järjehoidja",
    "Save your passwords with one click." : "Salvesta oma salasõnad ühe klõpsuga.",
    "Drag below button to your bookmark toolbar." : "Lohista järgnev nupp oma järjehoidjate tööriistaribale.",
    "Delete vault" : "Kustuta andmehoidla",
    "Vault password" : "Andmehoidla salasõna",
    "This process is irreversible" : "Seda tegevust pole võimalik tagasi pöörata",
    "Delete my precious passwords" : "Kustuta minu väärtuslikud salasõnad",
    "Deleting {{password}} …" : "Kustutan: {{password}} …",
    "Yes, delete my precious passwords" : "Jah, kustuta minu väärtuslikud salasõnad",
    "Import type" : "Importimise tüüp",
    "Import" : "Impordi",
    "Read progress" : "Lugemise edenemine",
    "Upload progress" : "Üleslaadimise edenemine",
    "Private Key" : "Privaatvõti",
    "Public key" : "Avalik võti",
    "Key size" : "Võtme pikkus",
    "Save keys" : "Salvesta võtmed",
    "Generate sharing keys" : "Loo jagamisvõtmed",
    "Generating sharing keys" : "Loon jagamisvõtmeid",
    "The password tool scans your password, calculates average cracking time, listing those below the threshold" : "Analüüsitarvik uurib sinu salasõna ning arvutab välja keskmise murdmisaja ning näitab neid lävendi all",
    "Minimum password stength" : "Salasõna miinimutugevus",
    "Start scan" : "Alusta uurimist",
    "Result" : "Tulemus",
    "A total of {{scan_result}} weak credentials were found." : "Leidsin kokku {{scan_result}} nõrka salasõna.",
    "Score" : "Tulemus",
    "Action" : "Tegevus",
    "Search users …" : "Otsi kasutajaid…",
    "Missing users? Only users that have vaults are shown." : "Kasutajaid on puudu? Kuvame vaid kasutajaid, kellele kuulub andmehoidlaid.",
    "Cyphering" : "Šifreerin",
    "Uploading" : "Üleslaadimisel",
    "User" : "Kasutaja",
    "Crypto time" : "Krüptimise aeg",
    "Total time spent encrypting" : "Krüptimisele kulutatud aega kokku",
    "Read" : "Lugemine",
    "Write" : "Kirjutamine",
    "Files" : "Failid",
    "Revisions" : "Versioonid",
    "Pending" : "Ootel",
    "Enable link sharing" : "Luba lingiga jagamine",
    "Share until date" : "Jagamise lõppkuupäev",
    "Expire after views" : "Märgi aegunuks pärast nii paljusid vaatamisi",
    "Click \"Share\" first" : "Esmalt klõpsi linki „Jaga“",
    "Show files" : "Näita faile",
    "Details" : "Üksikasjad",
    "Hide details" : "Peida üksikasjad",
    "Password score" : "Hinnang salasõnale",
    "Cracking times" : "Salasõnamurdmise ajad",
    "100/hour" : "100/tunnis",
    "Throttled online attack" : "Piiratud rünnak võrgust",
    "10/second" : "10/sekundis",
    "Unthrottled online attack" : "Piiramata rünnak võrgust",
    "10k/second" : "10k/sekundis",
    "Offline attack, slow hash, many cores" : "Rünnak kohalikus seadmes, aeglane räsiarvutus, kasutusel mitu protsessori tuuma ",
    "10B/second" : "10B/sekundis",
    "Offline attack, fast hash, many cores" : "Rünnak kohalikus seadmes, kiire räsiarvutus, kasutusel mitu protsessori tuuma ",
    "Match sequence" : "Vastavuste jada",
    "See match sequence" : "Vaata vastavuste jada tekkimist",
    "Pattern" : "Muster",
    "Matched word" : "Tuvastatud sõna",
    "Dictionary name" : "Sõnastiku nimi",
    "Rank" : "Populaarsus",
    "Reversed" : "Tagurpidi",
    "Guesses" : "Ennustuskordi",
    "Base guesses" : "Baasennustusi",
    "Uppercase variations" : "Suurtähtedes variante",
    "l33t-variations" : "Leetkeeles variante",
    "Showing revisions of \"{{label}}\"" : "Kuvan eri versioone: {{label}}",
    "Revision of" : "Versioon ajatempliga",
    "by" : "kasutajalt",
    "No revisions found." : "Varasemaid versioone ei leidu.",
    "Label" : "Nimi",
    "Restore revision" : "Taasta versioon",
    "Delete revision" : "Kustuta versioon",
    "Edit credential" : "Muuda salasõnakirjet",
    "Create new credential" : "Loo uus salasõnakirje",
    "Save" : "Salvesta",
    "Cancel" : "Loobu",
    "Settings" : "Seadistused",
    "Share credential {{credential}}" : "Jaga kasutajakontot/salasõna: {{credential}}",
    "Unshare" : "Lõpeta jagamine",
    "Showing deleted since" : "Näitan kustutamisi alates",
    "Beginning" : "Alates",
    "Showing {{number_filtered}} of {{credential_number}} credentials" : "Näitan salasõnakirjeid: {{number_filtered}} / {{credential_number}} ",
    "Search for credential …" : "Otsi salasõna või kasutajakontot…",
    "Account" : "Konto",
    "Password" : "Salasõna",
    "OTP" : "Ühekordne salasõna",
    "Email" : "E-post",
    "URL" : "Võrguaadress",
    "Notes" : "Märkmed",
    "Expiry time" : "Aegumine",
    "Changed" : "Muudetud",
    "Created" : "Loodud",
    "Edit" : "Muuda",
    "Delete" : "Kustuta",
    "Share" : "Jaga",
    "Recover" : "Taasta",
    "Destroy" : "Hävita",
    "Use regex" : "Kasuta regulaaravaldist",
    "You have incoming share requests." : "Sulle on saabunud jagamispalveid.",
    "If you want to put the credential in another vault," : "Kui soovid salasõnakirjet lisada teise andmehoidlasse, siis",
    "log out of this vault and log into the vault you want the shared credential in." : "logi sellest andmehoidlast välja ja logi sisse sellesse andmehoidlasse, kus tahad salasõnakirjet näha.",
    "Permissions" : "Õigused",
    "Received from" : "Saatja",
    "Date" : "Kuupäev",
    "Accept" : "Nõustu",
    "Decline" : "Keeldu",
    "You have {{session_time}} left before logout." : "Väljalogimiseni on aega {{session_time}}.",
    "Your vault has been locked for {{time}} because of {{tries}} failed attempts!" : "{{tries}} ebaõnnestunud katse tõttu on sinu andmehoidla lukustatud {{time}} korda!",
    "Hello there!" : "No terekest kah!",
    "It does not seem that you have any passwords. Do you want to add one?" : "Tundub, et sul pole salasõnu. Tahaksid sa mõne lisada?",
    "You don't have good credentials" : "Sul pole häid salasõnakirjeid",
    "You don't have medium credentials" : "Sul pole keskmisi salasõnakirjeid",
    "You don't have bad credentials" : "Sul pole nõrku salasõnakirjeid",
    "You don't have expired credentials" : "Sul pole aegunud salasõnakirjeid",
    "You don't have deleted credentials" : "Sul pole kustutatud salasõnakirjeid",
    "There are no credentials with your selected tags" : "Sinu valitud siltidele ei vasta salasõnakirjeid",
    "There are no credentials matching" : "Sobivaid kasutajakontosid/salasõnu pole",
    "Last accessed" : "Viimati avatud",
    "Never" : "Mitte kunagi",
    "No vaults found, why not create one?" : "Ühtegi andmehoidlat ei leidu, seega on ju hea hetk ühe sellise loomiseks?",
    "Password strength must be at least: {{strength}}" : "Salasõna tugevus peab olema vähemalt: {{strength}}",
    "Please give your new vault a name." : "Palun sisesta oma uue andmehoidla nimi.",
    "Repeat vault password" : "Korda andmehoidla salasõna",
    "Your sharing keys will have a strength of 1024 bit, which you can change in \"Settings\" later." : "Sinu jagamisvõtmed on 1024-bitise tugevusega ning seda võid seadistustest ka muuta",
    "Create vault" : "Tee andmehoidla",
    "Go back to vaults" : "Tagasi andmehoidlate juurde",
    "Please input the password for" : "Palun sisesta salasõna",
    "Set this vault as the default." : "Määra see andmehoidla vaikimisi hoidlaks.",
    "Log into this vault automatically." : "Logi sellesse andmehoidlasse automaatselt.",
    "Log out of this vault automatically after:" : "Logi sellest andmehoidlast automaatselt välja peale:",
    "30 minutes" : "30 minutit",
    "60 minutes" : "60 minutit",
    "90 minutes" : "90 minutit",
    "3 hours" : "3 tundi",
    "8 hours" : "8 tundi",
    "Decrypt vault" : "Dekrüpti andmehoidla",
    "Seems you lost the vault password and you're unable to log in." : "Tundub, et oled kaotanud andmehoidla salasõna ega saa sisse logida.",
    "If you want this vault to be removed you can request that here." : "Kui tahad andmehoidlat eemaldada, siis saad seda siin paluda.",
    "An admin then accepts or declines the request" : "Seejärel peakasutaja nõustub päringuga või lükkab selle tagasi",
    "After an admin destroys this vault, all credentials in it will be lost" : "Kui peakasutaja kustutab andmehoidla, siis kõik seal leiduvad kontod ja salasõnad lähevad kaotsi.",
    "Reason for requesting deletion (optional):" : "Kustutuspalve põhjus (kui soovid lisada):",
    "Request vault destruction" : "Palu andmehoidla kustutamist",
    "Yes, request an admin to destroy this vault" : "Jah, palun peakasutajat et ta kustutaks selle andmehoidla",
    "Cancel destruction request" : "Tühista andmehoidla kustutamispalve",
    "Vault destruction requested" : "Andmehoidla kustutamispalve on saadetud",
    "Request removed" : "Andmehoidla kustutamispalve on eemaldatud",
    "Destruction request pending" : "Andmehoidla kustutamispalve on ootel",
    "Warning! Adding credentials over HTTP is insecure!" : "Hoiatus! Salasõnade lisamine üle HTTP-ühenduse on väga ebaturvaline!",
    "Logged into {{vault_name}}" : "Sisselogitud andmehoidlasse: {{vault_name}}",
    "Change vault" : "Muuda andmehoidlat",
    "Deleted credentials" : "Kustutatud salasõnakirjed",
    "Logout" : "Logi välja",
    "Donate" : "Anneta",
    "Show All" : "Näita kõiki",
    "Tags" : "Sildid",
    "Search Tags" : "Otsi silte",
    "Good Strength" : "Tugev salasõna",
    "Medium Strength" : "Keskmine salasõna",
    "Bad Strength" : "Nõrk salasõna",
    "Expired" : "Aegunud",
    "Compromised" : "Rääderdatud",
    "Filter Tags" : "Filtreeri silte",
    "Simple Navigation" : "Lihtne menüü",
    "Someone has shared a credential with you." : "Keegi on jaganud sinuga salasõna andmeid.",
    "Click here to request it" : "Selle kuvamiseks klõpsi siin",
    "Loading" : "Laadin",
    "Oops! Credential not found. Maybe it expired." : "Vaat kus lops! Salasõnakirjet ei leidu. Aga ta näiteks võib olla aegunud.",
    "Mark as Compromised" : "Märgi rääderdatuks",
    "Compromised!" : "Rääderdatud!",
    "This password is compromised. You can only remove this warning by changing the password." : "See salasõna on rääderdatud ehk kahjustatud salajasusega. Saad selle teavituse eemaldada vaid salasõna vahetamise teel.",
    "email" : "e-post",
    "Description" : "Kirjeldus",
    "Custom Search:" : "Kohandatud otsing:",
    "Revert to defaults" : "Taasta vaikimisi valikud",
    "Your credential \"%s\" expired, click here to update the credential." : "Sinu „%s“ salasõnakirje on aegunud, uuendamiseks klõpsi siin.",
    "Remind me later" : "Tuleta mulle hiljem meelde",
    "Ignore" : "Eira",
    "%s shared \"%s\" with you. Click here to accept" : "%s jagas sinuga „%s“ salasõna. Klõpsi vastuvõtmiseks",
    "%s has declined your share request for \"%s\"." : "%s on keeldunud sinu „%s“ salasõna jagamispalvest.",
    "%s has accepted your share request for \"%s\"." : "%s on nõustunud sinu „%s“ salasõna jagamispalvega.",
    "Unable to get version info" : "Ei suuda tuvastada versiooni teavet",
    "Passman" : "Passman",
    "Passwords" : "Salasõnad",
    "Passman is a full-featured password manager." : "Passman on täismahuline salasõnahaldur.",
    "**Passman** is a full-featured, open source password manager for Nextcloud.\n\n## Features:\n- Vaults\n- Vault key is never sent to the server\n- Browser extension for easy access to passwords\n- Android app for on the road access\n- Credentials are client side encrypted with 256bit AES\n- Credentials are server side encrypted with 256bit AES\n- Ability to add custom fields to credentials\n- Built-in OTP (One Time Password) generator\n- Password analyzer\n- Share passwords internally and via link in a secure manner.\n- Import from various password managers (KeePass, LastPass, DashLane, ZOHO, Clipperz.is)\n\n## Demo\nFor a demo of this app visit [https://demo.passman.cc](https://demo.passman.cc)" : "Passman on täismahuline salasõnahaldur.\n\nFunktsionaalsused:\n- Andmehoidlad\n- Andmehoidla võtit ei saadeta kunagi serverile\n- Brauserilaiendus lihtsaks ligipääsuks salasõnadele\n- Androidi rakendus, mis tagab, et salasõnad on alati kaasas\n- Andmed on kliendi poolel krüptitud 256-bitise AES-algoritmiga\n- Andmed on serveris krüptitud 256-bitise AES-algoritmiga\n- Andmekirjetele saad lisada täiendavaid välju\n- Sisseehitatud ühekordsete salasõnade (OTP - One Time Password) generaator\n- Salasõnade analüüsija\n- Võimalus salasõnu turvalisel viisil serverisiseselt jagada lingiga\n- Importimise võimalus teistest salasõnahalduritest (KeePass, LastPass, DashLane, ZOHO, Clipperz.is)\n\nNäidisrakenduse leiad siit: [https://demo.passman.cc](https://demo.passman.cc)",
    "Passman Settings" : "Passmani seadistused",
    "GitHub version:" : "GitHubi versioon:",
    "Local version:" : "Kohalik versioon:",
    "A newer version of Passman is available" : "Saadaval on uus Passmani versioon",
    "Password sharing" : "Salasõna jagamine",
    "Credential mover" : "Salasõnade teisaldaja",
    "Vault destruction requests" : "Andmehoidla kustutamispäringud",
    "Check for new versions" : "Kontrolli uusi versioone",
    "Enable HTTPS check" : "Lülita sisse httpsi olemasolu kontroll",
    "Disable context menu" : "Lülita kontekstimenüü välja",
    "Disable JavaScript debugger" : "Lülita JavaScripti siluja välja",
    "Enable global search" : "Kasuta üldotsingut",
    "Minimum vault key strength" : "Andmehoidla võtme minimaalne pikkus",
    "Allow users on this server to share passwords with a link" : "Luba selle serveri kasutajatel jagada salasõnu lingi vahendusel",
    "Allow users on this server to share passwords with other users" : "Luba selle serveri kasutajatel jagada salasõnu teiste  kasutajatega",
    "Move credentials from one account to another" : "Tõsta salasõnakirjed ühelt kontolt teisele",
    "Source account" : "Lähtekonto",
    "Search for a user" : "Otsi kasutajat",
    "Destination account" : "Sihtkonto",
    "Move" : "Teisalda",
    "Credentials moved!" : "Salasõnakirjed on ümber tõstetud!",
    "An error occurred!" : "Tekis viga!",
    "Requests to destroy vault" : "Päringud andmehoidla kustutamiseks",
    "Request ID" : "Päringu tunnus",
    "Requested by" : "Päringu tegija",
    "Reason" : "Põhjus",
    "Click here to request\n\t\t\t\t\tit" : "Pärimiseks klõpsi siin:\n\t\t\t\t\t",
    "E-mail" : "E-post",
    "Expires:" : "Aegub:",
    "Connection to server lost" : "Ühendus serveriga katkes",
    "Problem loading page, reloading in 5 seconds" : "Viga lehe laadimisel, laadin uuesti 5 sekundi pärast",
    "Saving …" : "Salvestamisel …",
    "Dismiss" : "Jäta vahele",
    "seconds ago" : "sekundit tagasi"
},
"nplurals=2; plural=(n != 1);");
