OC.L10N.register(
    "passman",
    {
    "Error while saving field" : "Kļūda, saglabājot lauku",
    "A Passman item has been renamed" : "Passman vienums tika pārdēvēts",
    "You created %1$s" : "Tu izveidoji %1$s",
    "%1$s has been updated by %2$s" : "%1$s ir atjauninājis %2$s",
    "You updated %1$s" : "Tu atjaunināji %1$s",
    "%3$s has renamed %1$s to %2$s" : "%3$s pārsauca %1$s uz %2$s",
    "You renamed %1$s to %2$s" : "Tu pārdēvēji %1$s uz %2$s",
    "You deleted %1$s" : "Tu izdzēsi %1$s",
    "You recovered %1$s" : "Tu atguvi %1$s",
    "%1$s has been permanently deleted by %2$s" : "%1$s neatgriezeniski izdzēsa %2$s",
    "You permanently deleted %1$s" : "Tu neatgriezeniski izdzēsi %1$s",
    "%s has been shared with a link" : "%s koplietots izmantojot saiti",
    "Passwords" : "Paroles",
    "Incorrect vault password!" : "Nepareiza glabātavas parole.",
    "Passwords do not match" : "Paroles nesakrīt",
    "General" : "Vispārīgi",
    "Custom Fields" : "Pielāgotie lauki",
    "Error loading file" : "Kļūda, ielādējot datni",
    "Credential created!" : "Akreditācijas izveidota!",
    "Credential deleted" : "Akreditācijas dati izdzēsti",
    "Credential updated" : "Akreditācijas dati atjaunināti",
    "Credential recovered" : "Akreditācijas dati atjaunoti",
    "Credential destroyed" : "Akreditācijas dati iznīcināti",
    "Invalid QR code" : "Nepareizs QR kods",
    "Starting export" : "Uzsāk izgūšanu",
    "Done" : "Pabeigts",
    "Adding {{credential}}" : "Pievienošana {{credential}}",
    "Added {{credential}}" : "Pievienots {{credential}}",
    "Settings saved" : "Iestatījumi saglabāti",
    "General settings" : "Vispārīgi iestatījumi",
    "Password settings" : "Paroles iestatījumi",
    "Export credentials" : "Izgūt piekļuves datus",
    "Sharing" : "Koplietošana",
    "Share with users and groups" : "Koplietot ar lietotājiem vai grupām",
    "Share link" : "Koplietot saiti",
    "Saved!" : "Saglabāts!",
    "Good" : "Labs",
    "Strong" : "Stipra",
    "Copy to clipboard" : "Kopēt starpliktuvē",
    "Copied to clipboard!" : "Nokopēts starpliktuvē!",
    "Generate password" : "Izveidot paroli",
    "Copy password to clipboard" : "Starpliktuvē kopēt paroli",
    "Password copied to clipboard!" : "Parole nokopēta starpliktuvē!",
    "Complete" : "Pabeigta",
    "Username" : "Lietotājvārds",
    "Repeat password" : "Atkārtot paroli",
    "Get icon from page" : "Iegūt ikonu no lapas",
    "Field label" : "Lauka nosaukums",
    "Field value" : "Lauka vērtība",
    "Choose a file" : "Izvēlēties datni",
    "Text" : "Teksts",
    "File" : "Datne",
    "Add" : "Pievienot",
    "Value" : "Vērtība",
    "Type" : "Veids",
    "Actions" : "Darbības",
    "Empty" : "Tukšs",
    "Filename" : "Datnes nosaukums",
    "Upload date" : "Augšupielādes datums",
    "Size" : "Izmērs",
    "Current OTP settings" : "Pašreizējāie OTP iestatījumi",
    "Issuer" : "Izdevējs",
    "Secret" : "Noslēpums",
    "Expiration date" : "Derīguma beigu datums",
    "No expiration date set" : "Nav iestatīts derīguma beigu datums",
    "Disabled" : "Atspējots",
    "Day(s)" : "Diena(s)",
    "Week(s)" : "Nedēļa(s)",
    "Month(s)" : "Mēnesis(ši)",
    "Year(s)" : "Gads(i)",
    "Password generation settings" : "Paroles veidošanas iestatījumi",
    "Password length" : "Paroles garums",
    "Use uppercase letters" : "Lietot lielos burtus",
    "Use lowercase letters" : "Jāizmanto mazie burti",
    "Use numbers" : "Lietot numurus",
    "Use special characters" : "Jāizmanto īpašās rakstzīmes",
    "Export type" : "Izgūšanas veids",
    "Export" : "Izgūt",
    "Enter vault password to confirm export." : "Jāievada glabātavas parole, lai apstiprinātu izgūšanu.",
    "Rename vault" : "Pārdēvēt glabātavu",
    "Change" : "Mainīt",
    "Total progress" : "Kopējā norise",
    "About Passman" : "Par Passman",
    "Version" : "Versija",
    "Bookmarklet" : "Grāmatzīmes",
    "Vault password" : "Glabātavas parole",
    "Import type" : "Ievietošanas veids",
    "Import" : "Ievietot",
    "Upload progress" : "Augšupielādes norise",
    "Private Key" : "Privāta atslēga",
    "Public key" : "Publiska atslēga",
    "Key size" : "Atslēgas izmērs",
    "Save keys" : "Saglabāt atslēgas",
    "Generate sharing keys" : "Veidot koplietošanas atslēgas",
    "Minimum password stength" : "Mazākais pieļaujamais paroles stiprums",
    "Score" : "Vērtība",
    "Action" : "Darbība",
    "Uploading" : "Augšupielāde",
    "User" : "Lietotājs",
    "Read" : "Lasīt",
    "Write" : "Rakstīt",
    "Files" : "Datnes",
    "Pending" : "Gaida",
    "Enable link sharing" : "Iespējot saites kopīgošana",
    "Share until date" : "Koplietot līdz datumam",
    "Show files" : "Rādīt datnes",
    "Details" : "Informācija",
    "Hide details" : "Paslēpt informāciju",
    "Password score" : "Paroles punktu skaits",
    "Matched word" : "Atbilstošs vārds",
    "Dictionary name" : "Vārdnīcas nosaukums",
    "Rank" : "Rangs",
    "Reversed" : "Apvērsta",
    "l33t-variations" : "l33t-variācijas",
    "by" : "līdz",
    "Label" : "Iezīme",
    "Restore revision" : "Atjaunot revīziju",
    "Save" : "Saglabāt",
    "Cancel" : "Atcelt",
    "Settings" : "Iestatījumi",
    "Unshare" : "Pārtraukt dalīšanos",
    "Account" : "Konts",
    "Password" : "Parole",
    "OTP" : "OTP",
    "Email" : "E-pasts",
    "URL" : "URL",
    "Notes" : "Piezīmes",
    "Changed" : "Mainīts",
    "Created" : "Izveidots",
    "Edit" : "Labot",
    "Delete" : "Izdzēst",
    "Share" : "Koplietot",
    "Recover" : "Atgūt",
    "Destroy" : "Iznīcināt",
    "Use regex" : "Izmantot regex",
    "Permissions" : "Atļaujas",
    "Received from" : "Saņemts no",
    "Date" : "Datums",
    "Accept" : "Akceptēt",
    "Decline" : "Noraidīt",
    "You don't have deleted credentials" : "Tev nav izdzēstu piekļuves datu",
    "Last accessed" : "Pēdējo reizi piekļūts",
    "Never" : "Nekad",
    "Password strength must be at least: {{strength}}" : "Paroles stiprumam ir jābūt vismaz: {{strength}}",
    "Please input the password for" : "Lūgums ievadīt paroli",
    "Deleted credentials" : "Izdzēsti akreditācijas dati",
    "Logout" : "Atteikties",
    "Donate" : "Ziedot",
    "Tags" : "Birkas",
    "Search Tags" : "Meklēt birkas",
    "Click here to request it" : "Klikšķināt šeit, lai to pieprasītu",
    "Loading …" : "Ielādē…",
    "email" : "e-pasts",
    "Description" : "Apraksts",
    "Your credential \"%s\" expired, click here to update the credential." : "Akreditācijas datiem \"%s\" beidzies derīgums. Jāklikšķina šeit, lai atjauninātu akreditācijas datus.",
    "Remind me later" : "Atgādināt vēlāk",
    "Ignore" : "Neņemt vērā",
    "%s shared \"%s\" with you. Click here to accept" : "%s ar Tevi kopīgoja \"%s\". Jāklikšķina šeit, lai pieņemtu",
    "Passman" : "Passman",
    "Unable to get version info" : "Nevar iegūt versijas informāciju",
    "Passman Settings" : "Passman iestatījumi",
    "Check for new versions" : "Pārbaudīt, vai nav jaunas versijas",
    "Enable HTTPS check" : "Iespējot HTTPS pārbaudi",
    "Disable context menu" : "Atspējot konteksta izvēlni",
    "Allow users on this server to share passwords with a link" : "Šajā serverī ļaut lietotājiem kopīgot paroles ar saiti",
    "Allow users on this server to share passwords with other users" : "Šajā serverī ļaut lietotājiem kopīgot paroles ar citiem lietotājiem",
    "Source account" : "Avota konts",
    "Destination account" : "Adresāta konts",
    "An error occurred!" : "Gadījās kļūda!",
    "Request ID" : "Pieprasījuma ID",
    "Requested by" : "Pieprasa",
    "Reason" : "Iemesls",
    "E-mail" : "E-mail",
    "Connection to server lost" : "Zaudēts savienojums ar serveri",
    "Problem loading page, reloading in 5 seconds" : "Problēma ielādējot lapu, pārlādēšana pēc 5 sekundēm",
    "Saving …" : "Saglabā ...",
    "Dismiss" : "Atmest",
    "seconds ago" : "pirms vairākām sekundēm"
},
"nplurals=3; plural=(n%10==1 && n%100!=11 ? 0 : n != 0 ? 1 : 2);");
