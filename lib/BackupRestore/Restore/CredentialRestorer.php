...
/**
 * @throws InvalidBackupException when a portable credential cannot be encrypted
 */
readonly class CredentialRestorer implements SectionRestorer {
	...
	public function section(): string {
		return BackupArchive::SECTION_CREDENTIALS;
	}
	...
	public function restore(BackupArchive $archive, RestoreContext $context): void {
		...
		foreach ($archive->section($section)->rows as $row) {
			$newVaultId = $context->vaultId($row['vault_id'] ?? null);
			if ($newVaultId === null) {
				$context->result->skip($section, RestoreWarnings::vaultOfCredentialMissing(
					BackupRow::readString($row, 'guid'),
					BackupRow::readInt($row, 'vault_id')
				));
				continue;
			}
			$row['vault_id'] = $newVaultId;

			if ($archive->manifest->isPortable()) {
				$row = $this->encryptionApplier->credentialRow($row);
			}

			$existingId = $context->isMerge() ? $this->lookup->findCredential(BackupRow::readString($row, 'guid'))?->getId() : null;
			$credential = $this->entityWriter->store(
				$section,
